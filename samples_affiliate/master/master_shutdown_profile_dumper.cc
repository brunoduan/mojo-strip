...
MasterShutdownProfileDumper::MasterShutdownProfileDumper(
...
void MasterShutdownProfileDumper::WriteTracesToDisc() {
  ...
  base::Thread flush_thread("master_shutdown_trace_event_flush");
  flush_thread.Start();
  flush_thread.task_runner()->PostTask(
      FROM_HERE, base::BindOnce(&MasterShutdownProfileDumper::EndTraceAndFlush,
...
void MasterShutdownProfileDumper::EndTraceAndFlush(
    base::WaitableEvent* flush_complete_event) {
  base::trace_event::TraceLog::GetInstance()->SetDisabled();
  base::trace_event::TraceLog::GetInstance()->Flush(
      base::Bind(&MasterShutdownProfileDumper::WriteTraceDataCollected,
                 base::Unretained(this),
                 base::Unretained(flush_complete_event)));
}

// static
base::FilePath MasterShutdownProfileDumper::GetShutdownProfileFileName() {
...
void MasterShutdownProfileDumper::WriteTraceDataCollected(
    base::WaitableEvent* flush_complete_event,
...
bool MasterShutdownProfileDumper::IsFileValid() {
  return dump_file_ && (ferror(dump_file_) == 0);
}

void MasterShutdownProfileDumper::WriteString(const std::string& string) {
  WriteChars(string.data(), string.size());
}

void MasterShutdownProfileDumper::WriteChars(const char* chars, size_t size) {
  if (!IsFileValid())
    return;
...
void MasterShutdownProfileDumper::CloseFile() {
  if (!dump_file_)
    return;
