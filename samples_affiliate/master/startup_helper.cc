...
void StartMasterTaskScheduler() {
  auto task_scheduler_init_params =
      GetSamplesClient()->master()->GetTaskSchedulerInitParams();
  if (!task_scheduler_init_params)
    task_scheduler_init_params = GetDefaultTaskSchedulerInitParams();
  DCHECK(task_scheduler_init_params);

  // If a renderer lives in the master process, adjust the number of
  ...
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
    ...
    task_scheduler_init_params->foreground_worker_pool_params =
        base::SchedulerWorkerPoolParams(
            std::max(GetMinThreadsInSlavererTaskSchedulerForegroundPool(),
