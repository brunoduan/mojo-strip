...
// to a file before the master process exits.
...
class MasterShutdownProfileDumper {
 public:
  explicit MasterShutdownProfileDumper(const base::FilePath& dump_file_name);

  ~MasterShutdownProfileDumper();
 ...
 private:
  ...
  int blocks_;
  // For dumping the samples to disc.
  FILE* dump_file_;

  DISALLOW_COPY_AND_ASSIGN(MasterShutdownProfileDumper);
};
