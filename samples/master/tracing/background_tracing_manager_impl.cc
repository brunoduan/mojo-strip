...
#include "samples/master/tracing/background_startup_tracing_observer.h"
#include "samples/master/tracing/background_tracing_rule.h"
...
void BackgroundTracingManagerImpl::WhenIdle(
  ...
  CHECK(samples::MasterThread::CurrentlyOn(samples::MasterThread::UI));
...
bool BackgroundTracingManagerImpl::SetActiveScenario(
  ...
  std::unique_ptr<samples::BackgroundTracingConfigImpl> config_impl(
...
void BackgroundTracingManagerImpl::OnStartTracingDone(
  ...
  DCHECK_CURRENTLY_ON(samples::MasterThread::UI);
  for (auto* observer : background_tracing_observers_)
...
void BackgroundTracingManagerImpl::OnHistogramTrigger(
  ...
  if (!samples::MasterThread::CurrentlyOn(samples::MasterThread::UI)) {
    base::PostTaskWithTraits(
        FROM_HERE, {samples::MasterThread::UI},
...
void BackgroundTracingManagerImpl::BeginFinalizing(
  ...
  samples::TracingControllerImpl::GetInstance()->StopTracing(
      trace_data_endpoint);
}
...
void BackgroundTracingManagerImpl::AbortScenario() {
  ...
  samples::TracingControllerImpl::GetInstance()->StopTracing(nullptr);
...
TraceConfig BackgroundTracingManagerImpl::GetConfigForCategoryPreset(
  ...
  switch (preset) {
    ...
    case BackgroundTracingConfigImpl::CategoryPreset::BENCHMARK_STARTUP: {
      auto config =
          tracing::TraceStartupConfig::GetDefaultMasterStartupConfig();
    ...
    case BackgroundTracingConfigImpl::CategoryPreset::BENCHMARK_NAVIGATION: {
      auto config = TraceConfig(
          "benchmark,toplevel,ipc,base,master,navigation,omnibox,ui,shutdown,"
          "safe_browsing,task_scheduler,"
          "disabled-by-default-task_scheduler_diagnostics,"
          "disabled-by-default-system_stats,disabled-by-default-cpu_profiler",
          record_mode);
      // Filter only master process events.
