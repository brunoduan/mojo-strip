...
// syncing the "activated" state of field trials between master and child
// processes. Specifically, when a field trial is activated in the master, it
// also activates it in the child process and when a field trial is activated
// in the child process, it notifies the master process to activate it.
class ChildProcessFieldTrialSyncer {
 public:
  // ChildProcessFieldTrialSyncer constructor taking an externally owned
  // |observer| param that's responsible for sending IPCs to the master process
  ...
  // Initializes field trial state change observation and notifies the master
  ...
  // Handler for messages from the master process notifying the child process
